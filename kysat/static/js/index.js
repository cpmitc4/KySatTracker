...
//
//
//				index.js - Main javascript file for KYSat-2 satellite tracker
//
...
//
//	Main javascript file for satellite tracker, handles Cesium initialization, Satellite
//	loading, keeping positions calculated and controlling the interface
//		
//	Requires: jQuery, Cesium, Modernizr, require.js(all contained in ./static/js/ directory
//	SGP4 algorithms(all contained in ../js/ directory
//
//	Licensed under the 2-clause BSD License, refer to ../LICENSE.txt
//
//	More information: Refer to ../KYSAT2-README.txt, ../KOANSYS-README.txt or contact the 
//	authors
//
//  Search for "CHANGED KYS2" to find things that the KYSat-2 team changed and "TODO KYS2"
// 	for things that will have to change(like KYSat-2's NORAD ID) or may change
//
///////////////////////////////////////////////////////////////////////////////////////////
...
(function () {
	...
	//CHANGED KYS2 3/30/13 - Refactored to support per-satellite images
	//slightly less performant since we're using multiple images, but more flexible for our solution
        var satnum, max, billboard;
		var images = new Array();
		
		
		//Treat the following variables as constants
		
		//TODO KYS2: Change KYSAT2_NORAD_ID to KYSat-2's ID once it is known
		//Satellite NORAD ID constants
		var ISS_NORAD_ID = "25544";
		var KYSAT2_NORAD_ID = "39384";
		
		//Satellite icon constants
		var ISS_ICON = '/images/ISS.png';
		var KYSAT2_ICON = '/images/KYSat2.png';
		var GENERIC_SATELLITE_ICON = '/images/Satellite.png';

		var loaded = 0;
		
        satBillboards.removeAll(); // clear out the old ones
        for (satnum = 0, max = satData.length; satnum < max; satnum += 1) {
            billboard = satBillboards.add({imageIndex: satnum,
			...
			// To add more custom imagery, just add another condition here and compare "satData[satnum].noradId" with whatever NoradId you need a custom icon for and
			// set it as shown below.
			
			// The function is added to the onload event for each satellite because the way Cesium TextureAtlases work, they essentially create one big texture from 
			// multiple smaller ones and so it only has to use parts of the larger texture to load the individual satellite textures. To do this, you can't actually
			// add the images to the textureatlas until they've been loaded, so we wait until the onload event has occurred for all objects, then we create the texture
			// atlas and bind it to the satellites. If we don't wait until they're all loaded, none of them will show up.
			
			var image;
			if(satData[satnum].noradId == ISS_NORAD_ID)
			{
				image = new Image();
				image.onload = function () {
					loaded++;
					if(loaded == (satData.length))
					{
						imagesLoaded(images);
					}
				};
			...
			else if(satData[satnum].noradId == KYSAT2_NORAD_ID)
			{
				image = new Image();
				image.onload = function () {
					loaded++;
			...
			else
			{
				...
				image.src = GENERIC_SATELLITE_ICON;
				images[satnum] = image;
			}
	...
	{
			// HACK: this is called once all images have been loaded. If we don't wait until then, the TextureAtlas won't work right
			// Seems like there should be a better way to do this, but I couldn't find it.
			var textureAtlas = scene.getContext().createTextureAtlas({images: images}); // seems needed in onload()
...
/*
    ...
    document.getElementById('select_satellite_group').onchange = function () {
        orbitTraces.removeAll();
		// Changed KYS2 3/31/2013 to load 1 file with all our sats - to load multiple files you will need some sort of selector on your page for it and to just use a 
		// variable to choose the filename here
        getSatrecsFromTLEFile('tle/custom.txt');
	...
	//Changed KYS2 3/30/13 Now we're only loading 1 file with all our sats
    getSatrecsFromTLEFile('/tle/custom.txt');
    populateSatelliteSelector();
    populateSatelliteBillboard();
	...
	// Changed KYS2 so we can draw KYSat-2's orbit on startup. Technically we're drawing whichever object is the first in the TLE data file,
	// so make sure KYSat-2's is the first one.
